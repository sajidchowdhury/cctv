...
export default function NewReceiptPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        toast({ title: "Failed", description: data.error ?? "Could not create receipt.", variant: "destructive" });
      } else {
        toast({ title: "Receipt recorded", description: data.message });
        router.push("/ledger");
      }
    } finally {
      setSaving(false);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="New receipt"
        description="Customer money-in with FIFO invoice settlement (doc §4.5)."
        action={
        ...
        <CardContent>
          <form onSubmit={onSubmit} className="space-y-4 max-w-lg">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="amount">Amount received (BDT) *</Label>
            ...
            {/* Open invoices (FIFO auto-allocate) */}
            ...
            {customerId && invoices.length === 0 && (
              <p className="text-xs text-muted-foreground">No open invoices for this customer. Receipt will be stored as advance.</p>
            )}

            <Button type="submit" disabled={saving || !customerId || !amount}>
              {saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />} Save receipt
            </Button>
          </form>
        </CardContent>
      </Card>
